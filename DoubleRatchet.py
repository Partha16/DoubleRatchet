...
class DRatch(object):
    ...
    def exportPublicKeys(self):
        keys = {}
        keys['IdentityKey'] = self.IdentityKey.public_key()
        if self.init:
            keys['SignedPreKey'] = self.SignedPreKey.public_key()
            keys['OnetimePreKey'] = self.OnetimePreKey.public_key()
        else:
            keys['EphemeralKey'] = self.EphemeralKey.public_key()
        
        return keys
    
    def _x3dh_init(self, recv : dict):
        # perform the 4 Diffie Hellman exchanges (X3DH)
        dh1 = self.SignedPreKey.exchange(recv['IdentityKey'])
        dh2 = self.IdentityKey.exchange(recv['EphemeralKey'])
        dh3 = self.SignedPreKey.exchange(recv['EphemeralKey'])
        dh4 = self.OnetimePreKey.exchange(recv['EphemeralKey'])
    ...
    def _x3dh_resp(self, sendr):
        # perform the 4 Diffie Hellman exchanges (X3DH)
        dh1 = self.IdentityKey.exchange(sendr['SignedPreKey'])
        dh2 = self.EphemeralKey.exchange(sendr['IdentityKey'])
        dh3 = self.EphemeralKey.exchange(sendr['SignedPreKey'])
        dh4 = self.EphemeralKey.exchange(sendr['OnetimePreKey'])
        # the shared key is KDF(DH1||DH2||DH3||DH4)
        self.SharedKey = hkdf(dh1 + dh2 + dh3 + dh4, 32)
        print(f'[{self.name}]\tShared key:', b64(self.SharedKey))
    ...
    def send(self, recv, msg):
        ...
        print(f'[{self.name}]\tSending ciphertext to Alice:', b64(cipher))
